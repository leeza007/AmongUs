...
void menu::draw()
{
	...
	if (menu::opened)
	{
		if (ImGui::Begin("HypnoticHook"))
		{
			ImGui::Checkbox("Cheat switch", &menu::settings.cheat_switch);
			if (menu::settings.cheat_switch)
			{
				ImGui::Text("Game settings:");
				ImGui::Checkbox("Always be imposter", &menu::settings.always_be_imposter);
				ImGui::Checkbox("No kill cooldown time", &menu::settings.no_cooldown_time);
				ImGui::Checkbox("Speed", &menu::settings.speed);
				ImGui::SliderInt("Value", &menu::settings.speed_value, 1, 30, "%d");
				ImGui::Checkbox("Kill distance customizer", &menu::settings.kill_value);
			
				if (menu::settings.kill_value)
					ImGui::Combo("Kill distance type", &menu::settings.kill_distance, { "Short\0Medium\0Long" });
			...
			{
				ImGui::Text("Tick this checkbox once you're in game otherwise you will crash!");
				ImGui::Text("No need for unticking if you quit a game while using this.");
			}

			if (ImGui::Button("Show Settings Folder"))
				system("explorer.exe /e, configs");
		}
		ImGui::End();
	}
}
